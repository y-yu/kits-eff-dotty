...
/**
  * @note Add `TypeLambda` case to `summonCombinedTag`.
  */
final class TagMacro(using override val qctx: Quotes) extends InspectorBase {
  ...
  private def summonCombinedTag[T <: AnyKind: Type](typeRepr: TypeRepr): Expr[Tag[T]] =
    typeRepr.dealias match {
      ...
      // This is very very ad-hoc!!!
      case TypeLambda(tvars, bounds, AppliedType(ctor, args)) =>
        val ctorTag = createTagExpr(ctor.dealias.asType)
        ctorTag.asInstanceOf[Expr[Tag[T]]]
